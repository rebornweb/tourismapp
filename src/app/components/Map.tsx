...
interface GoogleMapsProps {
  onLocationChange: (lat: number, lng: number) => void; // Explicitly define the type of onLocationChange prop
}
...
const GoogleMaps: React.FC<GoogleMapsProps> =  ({ onLocationChange }) => {
  ...
  const initMap = async () => {
    ...
    autocomplete.addListener("place_changed", async () => {
      ...
      const placeNameElement = infowindowContent.querySelector('#place-name') as HTMLSpanElement;
      const placeAddressElement = infowindowContent.querySelector('#place-address') as HTMLSpanElement;
      
      if (placeNameElement && placeAddressElement) {
        placeNameElement.textContent = place.name || '';
        placeAddressElement.textContent = place.formatted_address || '';
      } else {
        console.error('Error accessing elements.');
      }
