...
// Define if we are in development mode
...
// Prepare Next.js app and then start the server
nextApp.prepare().then(() => {
  // Create an HTTP server and handle requests with Express and Next.js
...
app.get('/api/nearby/places', async (req, res) => {
  ...
  try {
    ...
    // Return the static JSON data to the client
    res.json(staticData);
...
app.post('/api/payment_intents', async (req, res) => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DUFFEL_API_ENDPOINT}/payments/payment_intents`, {
      method: 'POST',
      headers: {
